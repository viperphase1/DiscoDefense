...
namespace Technie.PhysicsCreator
{
	...
	public class ConstructionPlane
	{
		...
		/** Construct a plane with a translation but no rotation */
		...
		/** Construct a plane with the same orientation as the base plane, but with a position offset */
		...
		private void Init()
		{
			//if (normal.magnitude < 0.01f) // FIXME: This very occasionally happens for certain models. Needs to be properly debugged even if it seems benign.
			//	Debug.LogError("!");
	...
	public class RotatedBox
	{
		...
		{
			Gizmos.matrix = Matrix4x4.TRS(this.center, plane.rotation, Vector3.one);
	...
	public class RotatedBoxFitter
	{
		...
		public BoxDef Fit(Vector3[] hullVertices, int[] hullIndices)
		{
			...
			// Always include the default construction plane (no rotation) because often that's a sensible thing for boxes
			planes.Add(new ConstructionPlane(Vector3.zero));
