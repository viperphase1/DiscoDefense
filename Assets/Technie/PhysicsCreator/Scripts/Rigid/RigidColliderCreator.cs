...
namespace Technie.PhysicsCreator
{
	public class HullMapping
	{
		...
		public RigidColliderCreatorChild targetChild; // null if non-child
		public RigidColliderCreatorChild[] targetAutoGeneratedChilds; // null if no auto generated colliders

		public void AddAutoChild(RigidColliderCreatorChild newChild, MeshCollider newCollider)
		{
			if (newChild != null)
			{
				List<RigidColliderCreatorChild> childList = new List<RigidColliderCreatorChild>();
	...
	public class RigidColliderCreator : MonoBehaviour, ICreatorComponent
	{
		...
		{
#if UNITY_EDITOR
			Console.output.Log("Repaint from OnDestroy");
		...
		/** Remove all colliders, leave child objects and RigidColliderCreatorChild components for later recreation */
		...
		/** Removes all generated components - colliders and child objects with child colliders and RigidColliderCreatorChild on them */
		public void RemoveAllGenerated()
		{
			...
			foreach (HullMapping mapping in hullMapping)
			{
				...
				{
					foreach (RigidColliderCreatorChild child in mapping.targetAutoGeneratedChilds)
					{
		...
		private static bool IsDeletable(GameObject obj)
		{
			...
			foreach (Component comp in allComps)
			{
				if (comp is Transform
					|| comp is Collider
					|| comp is RigidColliderCreator
					|| comp is RigidColliderCreatorChild)
				{
					numIgnorable++;
				}
			}
		...
		private void CreateHullMapping()
		{
			...
			foreach (RigidColliderCreatorChild c in FindLocal<RigidColliderCreatorChild>())
			{
				if (!IsMapped(c))
				{
			...
			foreach (HullMapping mapping in hullMapping)
			{
				...
				{
					// Mapping has a child collider but no RigidColliderCreatorChild
					// Recreate the child component
					RigidColliderCreatorChild newChild = AddComponent<RigidColliderCreatorChild>(mapping.generatedCollider.gameObject);
		...
		private void CreateCollider<T>(Hull sourceHull) where T : Collider
		{
			...
			//	GameObject newChild = new GameObject(sourceHull.name);
				...
				RigidColliderCreatorChild childPainter = AddComponent<RigidColliderCreatorChild>(newChild);
		...
		private void UpdateCollider(Hull hull)
		{
			...
			else if (hull.type == HullType.ConvexHull)
			{
				...
				meshCollider.cookingOptions &= ~MeshColliderCookingOptions.EnableMeshCleaning; // Disable the 'mesh cleaning' flag as unity likes to collapse our nice accurate hulls into garbage.
#endif
			...
			else if (hull.type == HullType.FaceAsBox)
			{
				...
				if (hull.isChildCollider)
				{
					boxCollider.transform.localPosition = hull.faceBoxCenter;
		...
		public void SetAllMaxPlanes(int maxPlanes)
		{
#if UNITY_EDITOR
			foreach (Hull h in paintingData.hulls)
			{
				h.maxPlanes = maxPlanes;
			}
			EditorUtility.SetDirty(paintingData);
#endif
		}
		...
		public Hull FindSourceHull(RigidColliderCreatorChild child)
		{
			...
			foreach (HullMapping h in hullMapping)
			{
				...
				{
					foreach (RigidColliderCreatorChild hC in h.targetAutoGeneratedChilds)
					{
						if (hC == child)
							return h.sourceHull;
					}
			...
			*/
			...
			for (int i=0; i< clippedHulls.Length; i++)
			{
				...
				else if (hull.isChildCollider)
				{
					...
					RigidColliderCreatorChild painterChild = child.AddComponent<RigidColliderCreatorChild>();
			...
			// Rebuild the list of RigidColliderCreatorChild components
			// Rebuilding from scratch ensures that they're in the same order as the autoGeneratedColliders array
			List<RigidColliderCreatorChild> childMarkers = new List<RigidColliderCreatorChild>();
			foreach (MeshCollider col in autoColliders)
			{
				childMarkers.Add(col.GetComponent<RigidColliderCreatorChild>());
			}
